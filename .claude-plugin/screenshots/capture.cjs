...
// screenshot truth pass, then writes local ANSI captures for comparison.
...
// leaks through the live-git path.

const fs = require('node:fs');
const path = require('node:path');
const { execFileSync } = require('node:child_process');

const ROOT = process.env.FORGE_SCREENSHOT_ROOT || path.resolve(__dirname, '..', '..');
...
const harnessStdin = JSON.stringify({
  model: { display_name: 'Review Model', id: 'review-model' },
...
function sanitize(text) {
  ...
  if (rootName) t = t.replace(new RegExp(escapeRegExp(rootName), 'g'), 'my-app');
  ...
  // Model labels are generic in public screenshot fixtures.
  return t;
}
