...
### Changed
- **Tier regrade: 24 → 12 Pro-gated commands (25 Free / 12 Pro / 0 Team).** An objective rubric pass over all 37 commands (`_runs/s410/tier-regrade.md`) freed 14 previously-gated commands, corrected `/4ge`'s README row to Free, and aligned all seven redirect stubs with their targets. **Not breaking, stays a minor:** the gate has zero `require_()` enforcement call sites in any command body — it prints an upgrade prompt at most (honest friction, not a hard block), so no public invocation is removed. `/maintain` and `/resp4wn` move *into* the gate, but both stubs continue to resolve and their *targets* (`/outhouse`, `/respawn`) were always Pro — this aligns the stubs with the rename-with-stub doctrine, it does not remove a public surface. Per `docs/plugin-versioning.md` §2, expanding the free public surface is feature-bearing Changed (free consumers can newly invoke 14 commands) ⇒ at least minor; per §5.4 (Released = committed) 2.2.2 is committed to main, so this is a new section, not a fold-in.
  - **Now Free (14 formerly gated):** `/ship`, `/commit`, `/pr` (commodity git wrappers — the delivery loop first-run already recommends to free users); `/decide`, `/constraint`, `/releases`, `/signoff`, `/lint`, `/lounge` (file append/read utilities and legacy redirect); `/studio`, `/substrate` (demo/charm assets, zero professional depth); `/blueprint` (bootstrap; reconciles the gate with the live /4ge page's free single-project bullet); `/infra` (health command for the local memory stack the free tier already includes); `/hitchhiker` (redirect → free `/recall`). `/4ge` is also shown as Free in the README table; it was already ungated in code.
